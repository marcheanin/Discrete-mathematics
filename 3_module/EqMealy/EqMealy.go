...
func Init_canon_auto(delta [][]int, phi [][]string, q0 int) (delta1 [][]int, phi1 [][]string, yes bool) {
	...
	for i := 0; i < time; i++ {
		delta1[i], phi1[i] = make([]int, len(delta[0])), make([]string, len(delta[q0]))
	}
	for i := 0; i < time; i++ {
		yes = true
...
func AufenkampHohn(delta [][]int, phi [][]string, q0 int) ([][]int, [][]string) {
	...
	for i, x := range pi {
		if Exists(x) {
			...
			pi[i], h3[x] = len(h3), len(h3)
		}
	}

	delta1, phi1, q01 := make([][]int, m), make([][]string, m), 0
	for i := 0; i < m; i++ {
		delta1[i], phi1[i] = make([]int, len(delta[q0])), make([]string, len(delta[q0]))
	}
	...
	for i := 0; i < len(delta); i++ {
		...
		for j := 0; j < len(delta[q0]); j++ {
			delta1[q][j], phi1[q][j] = pi[delta[i][j]], phi[i][j]
		}
	}
...
func Read(n, m int) ([][]int, [][]string) {
	...
	for i := 0; i < n; i++ {
		a[i], b[i], h1[i] = make([]int, m), make([]string, m), -1
	}
	for i := 0; i < n; i++ {
		for j := 0; j < m; j++ {
			fmt.Scanf("%d", &a[i][j])
...
func main() {
	...
	//Compare
	if len(minimized_delta2) != len(minimized_delta3) || len(minimized_delta2[0]) != len(minimized_delta3[0]) {
		fmt.Printf("NOT EQUAL\n")
		os.Exit(1)
	}
	for i := 0; i < len(minimized_delta2); i++ {
		for j := 0; j < len(minimized_delta2[0]); j++ {
			if minimized_delta2[i][j] == minimized_delta3[i][j] {
				mark_delta++
			}
		}
	}
	for i := 0; i < len(minimized_phi2); i++ {
		for j := 0; j < len(minimized_phi2[0]); j++ {
			if minimized_phi2[i][j] == minimized_phi3[i][j] {
				mark_phi++
			}
		}
	}
	if mark_delta == len(minimized_delta2)*len(minimized_delta2[0]) && mark_phi == len(minimized_phi2)*len(minimized_phi2[0]) {
		fmt.Printf("EQUAL\n")
	} else {
		fmt.Printf("NOT EQUAL\n")
	}
